 ...
 return (req:AuthRequest,res:Response,next:NextFunction)=>{
  res.on('finish',async()=>{
   ...
   try{
    ...
    await pool.execute('INSERT INTO audit_logs (id,user_id,action,entity,entity_id,details,ip_address) VALUES (UUID(),?,?,?,?,?,?)',[req.user?.id||null,action,entity,req.params.id||req.params.tenderId||req.params.ticketId||null,JSON.stringify({method:req.method,path:req.originalUrl,body:redact(req.body),statusCode:res.statusCode}),ip]);
