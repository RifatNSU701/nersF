...
export const submitBid = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!vendorId) { res.status(401).json({ message: 'Unauthorized' }); return; }
...
export const getMyBids = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const [rows]: any[] = await pool.execute(`SELECT b.*, t.title, t.reference_no, t.closing_date FROM bids b JOIN tenders t ON b.tender_id = t.id WHERE b.vendor_id = ? ORDER BY b.created_at DESC`, [req.user?.id]);
...
export const getBidsByTender = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const [rows]: any[] = await pool.execute(`SELECT b.*, u.full_name as vendor_name, u.email FROM bids b JOIN users u ON b.vendor_id = u.id WHERE b.tender_id = ? ORDER BY b.price_per_mt ASC`, [tenderId]);
    res.status(200).json(rows);
